...
class SecretaryTest < Test::Unit::TestCase
  ...
  def test_load_locations_are_expanded_when_expand_paths_is_true
    ...
    expected_locations = [
      File.join(FIXTURES_PATH, "src", "interdependent_sources"),
      File.join(FIXTURES_PATH, "src", "foo"),
      File.join(FIXTURES_PATH, "src"),
      FIXTURES_PATH]
    assert_equal expected_locations, secretary.environment.load_path.map { |pathname| pathname.absolute_location }
  end
