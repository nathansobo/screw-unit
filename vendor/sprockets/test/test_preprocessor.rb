...
class PreprocessorTest < Test::Unit::TestCase
  ...
  def test_dependency_ordered_source_files
    require_file_for_this_test

    expected_source_file_order = [5, 3, 4, 2, 1].map do |n|
      source_file("/interdependent_sources/#{n}.js")
    end
    expected_source_file_order.push(source_file(file_for_this_test))


    assert_equal expected_source_file_order, preprocessor.dependency_ordered_source_files
  end
  ...
  def require_file(location)
    preprocessor.require(source_file(location))
  end

  def source_file(location)
    environment.find(location).source_file
  end
