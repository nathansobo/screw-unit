    ...
    def('examples_to_run', function() {
      if (Prefs.data.run_paths) {
        return Screw.map(Prefs.data.run_paths, function() {
          return Screw.global_description().runnable_at_path(this);
        })
      } else {
...
$(function() {
  ...
  var runner = Disco.build(Screw.Interface.Runner, {root: Screw.global_description(), runnable: Screw.Interface.examples_to_run()[0]})
  $('body').html(runner);
  runner.run();
});